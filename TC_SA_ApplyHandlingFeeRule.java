...
public class TC_SA_ApplyHandlingFeeRule extends DriverSetup {
	...
	public void setup() {				
		...
		System.out.println("\ninside setup() fn (TC_SA_ApplyHandlingFeeRule.java)");
		System.out.println("TC_SA_ApplyHandlingFeeRule.java execution starts now");
		System.out.println("\nGetting current test case name (TC_SA_ApplyHandlingFeeRule.java");
		System.out.println("-------------------------------");
		currentTestName = getTestName();
		System.out.println("\nTest Case name is :" +currentTestName+"\n");
		excelRead = new ExcelRead();
		commonUtility = new CommonUtility();
		System.out.println("\nTrying to create a Report file with Test Case name in /reports/xls (TC_SA_ApplyHandlingFeeRule.java");
		System.out.println("-----------------------------------------------------------------");
		...
		System.out.println ("pppppppp setup() fn (TC_SA_ApplyHandlingFeeRule.java)");
		MyWaitVar30Sec = new WebDriverWait(driver,30);
		MyWaitVar5Sec = new WebDriverWait(driver,5);
		System.out.println ("QQQQQQQQQ setup() fn (TC_SA_ApplyHandlingFeeRule.java)");

	}
	
		
	@DataProvider(name = "TC_SA_ApplyHandlingFeeRule")
	public Object[][] createData2() throws Exception {
	         
		System.out.println("inside TC_SA_ApplyHandlingFeeRule.java");
		System.out.println("\nTrying to to read input test data from input datasheet (TC_SA_ApplyHandlingFeeRule.java -->ExcelRead.java");
		System.out.println("--------------------------------------------------------");
	...
	@Test(dataProvider = "TC_SA_ApplyHandlingFeeRule")
	public void TC_SA_ApplyHandlingFeeRuleRule(String ToCity1, String Depart1,
...
List<WebElement> bookButtonClick =  driver.findElements(By.xpath("//*[starts-with(@id,'bookButtonId')]/button"));
	...
	for(int j = 0;j < 1; ++j) 
	{
		...
		// k=k+1;
		 ...
		 try
		 {
		 String visibleText = driver.findElement(By.xpath(".//*[@id='product_develop']/div[4]/div[2]/div[2]/div")).getText();
		...
		//if ( visibleText.equals("(Handling fee)"))
			if ( visibleText.contains("(Handling fee)"))
		{
		 ...
		 excelreadwrite.insertData(currentTestName,
			...
			else
			{
				 ...
				 excelreadwrite.insertFailedData(currentTestName,
						...
						"Checking whether Handling Fee shown in page3 for "+ m+"th product",
						"", true,
				"", "Handing Fee is NOT shown in page3. Make sure matching rule is present in DB ",
					"Handling Fee should be shown in page3, if matching rule is present");
					Assert.assertFalse(true,
							"In Fare matrix display actual text is not valid");
		...
		{
			System.out.println ("No Handling Fee appiled  ");
			 
			 excelreadwrite.insertFailedData(currentTestName,
						commonUtility.getcurrentDateTime(), "Apply FB module",
					"Checking whether Handling Fee shown in page3 for "+ m+"th product",
					"", true,
			"", "No Handing Fee amount shown in page3. Make sure matching rule is present in DB ",
				"Handling Fee should be shown in page3, if matching rule is present");
				Assert.assertFalse(true,
						"In Fare matrix display actual text is not valid");
		...
		// System.out.println ("removedCurrencySybmolHandFeeAmount:"+removedCurrencySybmolHandFeeAmount);
		 ...
		 String removedDecimalPointsHandFeeAmount0 = removedCommaHandFeeAmount.substring(0, 4);
		 //System.out.println ("removedDecimalPointsHandFeeAmount:"+removedDecimalPointsHandFeeAmount);
		 String removedDecimalPointsHandFeeAmount = removedDecimalPointsHandFeeAmount0.trim();
		 ...
		 if (removedDecimalPointsHandFeeAmount.equals(ExpAmount)) //ExpAmount)
		 {
			 ...
			 excelreadwrite.insertData(currentTestName,
						commonUtility.getcurrentDateTime(), "Apply FB Rule module",
							"Checking whether Handling Fee applied is correct or not for "+ m+"th product",
							"", true,
							"", "Handing Fee Applied is correct. "+removedDecimalPointsHandFeeAmount+" Applied",
		 ...
		 else
		 {
			 ...
			 excelreadwrite.insertFailedData(currentTestName,
						commonUtility.getcurrentDateTime(), "Apply FB module",
					"Checking whether Handling Fee amount is correct or not",
					"", true,
			"", "Handling Fee amount is NOT correct. Actual amount is "+removedDecimalPointsHandFeeAmount ,
				"Expected amount is "+ExpAmount);
				Assert.assertFalse(true,
						"In Fare matrix display actual text is not valid");
