...
public class TC_SA_ApplyUrgentFeeRule extends DriverSetup {
	...
	public void setup() {				
		...
		System.out.println("\ninside setup() fn (TC_SA_ApplyUrgentFeeRule.java)");
		System.out.println("TC_SA_ApplyUrgentFeeRule.java execution starts now");
		System.out.println("\nGetting current test case name (TC_SA_ApplyUrgentFeeRule.java");
		System.out.println("-------------------------------");
		currentTestName = getTestName();
		System.out.println("\nTest Case name is :" +currentTestName+"\n");
		excelRead = new ExcelRead();
		commonUtility = new CommonUtility();
		System.out.println("\nTrying to create a Report file with Test Case name in /reports/xls (TC_SA_ApplyUrgentFeeRule.java");
		System.out.println("-----------------------------------------------------------------");
		...
		System.out.println ("pppppppp setup() fn (TC_SA_ApplyUrgentFeeRule.java)");
		MyWaitVar30Sec = new WebDriverWait(driver,30);
		MyWaitVar5Sec = new WebDriverWait(driver,5);
		System.out.println ("QQQQQQQQQ setup() fn (TC_SA_ApplyUrgentFeeRule.java)");

	}
	
		
	@DataProvider(name = "TC_SA_ApplyUrgentFeeRule")
	public Object[][] createData2() throws Exception {
	         
		System.out.println("inside TC_SA_ApplyUrgentFeeRule.java");
		System.out.println("\nTrying to to read input test data from input datasheet (TC_SA_ApplyUrgentFeeRule.java -->ExcelRead.java");
		System.out.println("--------------------------------------------------------");
	...
	@Test(dataProvider = "TC_SA_ApplyUrgentFeeRule")
...
List<WebElement> bookButtonClick =  driver.findElements(By.xpath("//*[starts-with(@id,'bookButtonId')]/button"));
	...
	for(int j = 0;j < 2; ++j) 
	{
		...
		if ( visibleText.contains("(Urgent fee)"))
		{
		 System.out.println ("Urgent Fee appiled ");
		...
		{
			 System.out.println ("Urgent Fee NOT appiled Successfully ");
			 
			 excelreadwrite.insertFailedData(currentTestName,
						commonUtility.getcurrentDateTime(), "Apply FB module",
					"Checking whether Urgent Fee shown in page3 for "+ m+"th product",
					"", true,
			"", "Urgent Fee is NOT shown in page3. Make sure matching rule is present in DB ",
		...
		/*
		 ...
		 String handlingFeeAmount = driver.findElement(By.xpath(".//*[@id='product_develop']/div[4]/div[2]/div[2]/div/dl[4]/dt[2]")).getText();
		...
		// System.out.println ("removedCurrencySybmolHandFeeAmount:"+removedCurrencySybmolHandFeeAmount);
		 ...
		 String removedDecimalPointsHandFeeAmount0 = removedCommaHandFeeAmount.substring(0, 5);
		 ...
		 if (removedDecimalPointsHandFeeAmount.equals(ExpAmount)) //ExpAmount)
		 {
			 ...
			 System.out.println ("Urgent Fee Amount shown is correct ");
			 
			 excelreadwrite.insertData(currentTestName,
						commonUtility.getcurrentDateTime(), "Apply FB Rule module",
							"Checking whether Urgent Fee applied successfully for "+ m+"th product",
							"", true,
							"", "Urgent Fee amount applied is cocrrect. Amount applied is "+removedDecimalPointsHandFeeAmount,
		 ...
		 else
		 {
			 System.out.println ("Expected amount:"+ExpAmount);
			 System.out.println ("Actual amount:"+removedDecimalPointsHandFeeAmount);
			 System.out.println ("Urgent Fee Amount shown is NOT correct ");
			 
			 excelreadwrite.insertFailedData(currentTestName,
						commonUtility.getcurrentDateTime(), "Apply FB module",
					"Checking whether Urgent Fee applied successfully for "+ m+"th product",
					"", true,
			"", "Urgent Fee amount applied is NOT correct. Amount applied is "+removedDecimalPointsHandFeeAmount,
				"Expected amount is "+ExpAmount);
				Assert.assertFalse(true,
						"In Fare matrix display actual text is not valid");
