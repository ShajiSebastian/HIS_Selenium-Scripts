...
public class TC_SA_Choose_Arrival  extends DriverSetup {
	...
	@DataProvider(name = "TC_SA_Choose_Arrival")
	...
	@Test(dataProvider = "TC_SA_Choose_Arrival")
	...
	excelreadwrite.insertData(currentTestName, commonUtility.getcurrentDateTime(), "Choose Arrival Functionality ", "Checking for number of products in basic listing page", "Printing product count", true, "Same as Expected", "No: of products displayed in basic listing page:"+initialcount, "Products should be displayed in basic listing page");
	
	List<WebElement> prodcount= driver.findElements(By.xpath(".//*[@id='checkTable']"));
	int size= prodcount.size();
	
	List<WebElement> deptcount= driver.findElements(By.xpath(".//*[@class='chooseArr_active']"));
	...
	if (size==sizeDept)
	{
		...
		excelreadwrite.insertData(currentTestName, commonUtility.getcurrentDateTime(), "Choose Arrival Functionality ", "Checking for Choose arrival link", "Validating Choose Arrival link", true, "Same as Expected", "All products is having Choose Arrival link", "All products should have Choose Arrival link");
	}
	else
	{
		System.out.println("No: of products having no Choose departure link:"+diffCount);
		excelreadwrite.insertData(currentTestName, commonUtility.getcurrentDateTime(), "Choose Arrival Functionality ", "Checking for Choose arrival link", "Validating Choose Arrival link", true, "Same as Expected", "No: of products having no Choose arrival link:"+diffCount, "No Choose arrival option is available for missing segment product");
	}
	driver.findElement(By.xpath(".//*[@class='chooseDept_active']")).click();
	Thread.sleep(16000);
	excelreadwrite.insertData(currentTestName, commonUtility.getcurrentDateTime(), "Choose Arrival Functionality ", "Checking for onward segments", "Validating onward segments", true, "Same as Expected", "Onward segment is found", "Onward segment should be available");
	...
	if (initialcount==finalcount)
	{
		...
		excelreadwrite.insertData(currentTestName, commonUtility.getcurrentDateTime(), "Choose Arrival Functionality ", "Clicking on back to listing  button", "Navigating to listing page", true, "Same as Expected", "Product count is correct and is  "+finalcount, "Product count should be :"+initialcount);
	}
	else
	{
		System.out.println("Product count is wrong");
		excelreadwrite.insertFailedData(currentTestName, commonUtility.getcurrentDateTime(), "Choose Arrival Functionality ", "Verify whether product count is same after navigating back from page3 ", "Product count is wrong", true, "Not Same as Expected", "Product count is wrong", "Product count should be correct");
		Assert.assertFalse(true, "Product count is wrong");
			...
			catch(Exception e)
			{
                ...
                excelreadwrite.insertFailedData(currentTestName, commonUtility.getcurrentDateTime(), "Choose Arrival Functionality ", "Verify whether Choose Arrival Functionality is available ", "Choose Arrival Functionality is not available", true, "Not Same as Expected", "Choose Arrival Functionality is not available", "Choose Arrival  should be available");
				Assert.assertFalse(true, "Choose Arrival Functionality is not available");
