...
public class VerifySortByDeparture_OnwardEarlierToLater extends DriverSetup {
	...
	public void setup() {
		...
		System.out.println("\ninside setup() fn (VerifySortByDeparture_OnwardEarlierToLater.java)");
		System.out.println("VerifySortByDeparture_OnwardEarlierToLater.java execution starts now");
		System.out.println("\nGetting current test case name (VerifySortByDeparture_OnwardEarlierToLater.java");
		System.out.println("-------------------------------");
		currentTestName = getTestName();
		System.out.println("\nTest Case name is :" +currentTestName+"\n");
		excelRead = new ExcelRead();
		commonUtility = new CommonUtility();
		System.out.println("\nTrying to create a Report file with Test Case name in /reports/xls (VerifySortByDeparture_OnwardEarlierToLater.java");
		System.out.println("-----------------------------------------------------------------");
	...
	@DataProvider(name = "VerifySortByDeparture_OnwardEarlierToLater")
	public Object[][] createData2() throws Exception {

	         
		System.out.println("inside VerifySortByDeparture_OnwardEarlierToLater.java");
		System.out.println("\nTrying to to read input test data from input datasheet (VerifySortByDeparture_OnwardEarlierToLater.java -->ExcelRead.java");
		System.out.println("--------------------------------------------------------");
		...
		Object[][] retObjArr = excelRead.getTableArray(path, "TC",
				"VerifySortByDeparture_OnwardEarlierToLater");
	
		return (retObjArr);
	}

	//@Test(dataProvider = "VerifySortByDepartureEarlierToLater")
	@Test(dataProvider = "VerifySortByDeparture_OnwardEarlierToLater")
	public void Verifychoosedeparture(String ToCity1, String Depart1,
		...
		System.out.println("\nTrying to invoke HIS page to Execute 'VerifySortByDeparture_OnwardEarlierToLater' Test Case  (VerifySortByDepartureEarlierToLater.java)");
		System.out.println("----------------------------------------------------------------------\n");
...
*/
	...
	//	Thread.sleep(5000);
		...
		if (pageTitle.toLowerCase().trim().contains("H.I.S. Flight Search Results".toLowerCase().trim())) 
		{
			...
			System.out.println("----------------------------------");
			System.out.println("\nUpdating Report VerifySortByDeparture_OnwardEarlierToLater.xls in reports/xls");
		...
		System.out.println("\nback to VerifySortByDeparture_OnwardEarlierToLater.java ");
		...
		System.out.println ("\nVerifying the Functionality 'Sort By Departure-OnwardEarlierToLater'");
		System.out.println ("*************************************************\n");
		...
		Thread.sleep(2000); 
				...
				System.out.println ("\nSort by Departure option availalbe");
					System.out.println("\nUpdating Report VerifySortByDepartureEarlierToLater.xls in reports/xls");
					excelreadwrite.insertData(currentTestName,
						commonUtility.getcurrentDateTime(), "Sorting module",
				...
				{
					System.out.println("Sort by Departure option NOT availalbe");
	 				...
	 				excelreadwrite.insertFailedData(currentTestName,
							...
							"Checking the presence of Sort by Departure option",
							"", true,
					"", "Sor by Departure option NOT availalbe",
						"Sort by Departure option Should be available");
				...
				try
				{
				driver.findElement(By.xpath(".//*[@id='depOutSortDivEtoL']")).click();
				...
				for(depSortCount=1; depSortCount<= 3; depSortCount++)
			 	{
					Thread.sleep(3000);;
					
					WebElement departureTime = driver.findElement(By.xpath("(.//*[@id='totPrdtSizeDiv'])["+depSortCount+"]//div[@class='col-sm-4 travelFrom']//div[@class='travelTime']"));
			...
			{
				System.out.println ("\nSome issues found while trying to sort by Departure Onward earlier to Later: " +SortPriceError);
 				...
 				excelreadwrite.insertFailedData(currentTestName,
						...
						"Checking whether products sorted based on Outbound- earlier to later",
						"", true,
				"", "Some issues found while trying to sort by Price",
					"Products should be sorted based on Outbound- earlier to later");
 				Assert.assertFalse(true,"Products should be sorted based on Outbound- earlier to later");
